      ...
      int warn_if_readin)
{
  ...
  // handle mapped overlays
  if (section != NULL 
      && gdbarch_overlay_source_section_p(current_arch)
      && gdbarch_overlay_source_section(current_arch, data->fnd->get_name()) != NULL
      && gdbarch_overlay_source_section(current_arch, data->fnd->get_name()) != section) 
  {
    // have to search for the real matching per_cu_data in this case.
    for(const auto &unit : per_bfd->all_units) 
    {
      if (gdbarch_overlay_source_section(current_arch, unit->fnd->get_name()) == section)
      {
